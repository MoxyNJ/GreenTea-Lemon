...
选择一段文本的第一个字符：`::first-letter` ，如 `p::first-letter`

选择一段文本的第一行：`""first-line`，如 `#myPara::first-letter`

将在内容元素之前 / 之后插入一个额外的元素：`::before`、`::after` 在选中元素的内容（content） 之前 、 之后添加一个元素。

常见的伪元素有：`::selection`、`::placeholder`等
