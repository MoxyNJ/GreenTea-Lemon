...
<img src="images/1.assets/image-20230125170522211.png" alt="image-20230125170522211" style="zoom: 50%;" />
...
- **单向传递。**和React一样，props只可从父至子传递数据，子组件不可对接受的props进行修改。
...
- 形式：
  ...
  1. 子组件定义暴露的数据：`defineProps(['someValue'])`

  1. 子组件定义暴露的事件：`defineEmits(['update:someValue'])`

  1. 子组件在 `input` 元素上绑定数据和事件：`<input :value="someValue" @input=$emit('update:someVelue', $event.target.value)>`

  1. 父组件绑定v-model，将数据保存在temp变量中：`<MyComponent v-model:someValue="temp" />`
...
- 自定义修饰符：[实例](https://play.vuejs.org/#)
  - 可以自定义 v-model 的修饰符，已经存在的有：.trim、.number 等，可以自定义，注意流程：
    1. 子组件需定义暴露的数据、暴露的修饰符、暴露的事件，注意修饰符名称必须为：`xxxModifiers`，即 “数据名+Modifiers”。事件名称必须为：`update:xxx`。
    2. 子组件中定义：触发事件后调用一个回调函数，在回调函数中在判断 `props.modelModifiers.xxx`  的if 内书写逻辑。
    3. 父组件调用：`  <MyComponent v-model.xxx="myText" />`
