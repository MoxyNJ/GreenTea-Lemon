...
在同时按下鼠标两个按键的时候，会报出错误。因为所谓鼠标同时按下，其实也是分先后顺序的：一定是先按下某个按键，在这个键没有松开的时候，又按下了另一个按键。这时候，先后按下两个按键，会触发两次 mousedown 事件。就会同时触发两组 mousemove。

这时候，第一个被触发的mousedown事件中，event.buttons 
