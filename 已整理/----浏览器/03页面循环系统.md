...
![image-20220701114420430](images/03%E9%A1%B5%E9%9D%A2%E5%BE%AA%E7%8E%AF%E7%B3%BB%E7%BB%9F.assets/image-20220701114420430.png)
...
- 在下一个宏任务执行之前，必须要保证微任务队列已经被清空。



宏任务队列 macrotask queue ：ajax、setTimeout、setInterval、DOM监听、UI Rendering等

微任务队列 microtask queue ：Promise.#.then、 Mutation Observer API、queueMicrotask()等
...
- 使用 `queueMicrotask()`。类似 setTimeout 一样，不同的是它创建了一个微任务回调。
...
![image-20220701114432588](images/03%E9%A1%B5%E9%9D%A2%E5%BE%AA%E7%8E%AF%E7%B3%BB%E7%BB%9F.assets/image-20220701114432588.png)
...
Intel 15-10400：6核心 / 12线程。

- CPU的运算速度非常快，且有多个线程。它可以快速的在多个进程之间迅速的切换； 
- 当我们进程中的线程获取到时间片时，就可以快速执行我们编写的代码，对于用户是透明的。



JavaScript 是单线程的。

- 这就意味着 JavaScript的代码，在同一个时刻只能做一件事，如果这件事是非常耗时的，就意味着当前的线程就会被阻塞。

Chrome 浏览器是多进程的，每个进程中又有很多的线程，其中包括执行JavaScript代码的线程。

- JavaScript的代码执行是在一个单独的线程中执行。
- 所以真正耗时的操作，实际上并不是由JavaScript线程在执行的，
  而是交给其他线程其他线程可以来完成这个耗时的操作：比如网络请求、定时器。当耗时操作结束时，通过回调 Js 再接手接下来的工作。
...
Js 除了浏览器，还有 node 运行环境。1 个 node 进程也有 n 个线程。

- Js 线程、文件读取（I/O）线程、网络线程 ...

浏览器中的 EventLoop 是根据 HTML5 定义的规范来实现的，不同的浏览器可能会有不同的实现，而Node中是由 libuv实现的。libuv是一个多平台的专注于异步IO的库，它最初是为Node开发的，但是现在也被使用到Luvit、Julia、pyuv等其他地方。
...
![截屏2022-08-02 00.20.47](images/03%E9%A1%B5%E9%9D%A2%E5%BE%AA%E7%8E%AF%E7%B3%BB%E7%BB%9F.assets/%E6%88%AA%E5%B1%8F2022-08-02%2000.20.47.png)

事件循环像是一个桥梁，是连接着应用程序的 **JavaScript** 和 **系统调用** 之间的通道：

- 文件IO、数据库、网络IO、定时器、其他子进程等等，在完成各自操作后，都会各自的结果和回调函数放到事件循环（任务队列）中。
- 事件循环会不断的从任务队列中，按序取出事件（结果 + 回调函数）来执行；

一次完整的事件循环 Tick 有多个阶段：

- 定时器（Timers）：**定时器回调**。执行 setTimeout() 和 setInterval() 到期的回调函数。
- 待定回调（Pending Callback）：**操作系统回调**。某些系统操作的回调函数，比如 TCP 错误类型（TCP 连接时接收到 ECONNREFUSED 连接拒绝错误）
- idle, prepare：**系统内部使用** ，空闲 / 准备。 
- 轮询（Poll）：**IO操作**。检索新的 I/O 事件，执行与 I/O 相关的回调； 
- 检测（check）：**专用方法**。setImmediate() 回调函数在这里执行，该方法是 node API。
- 关闭的回调函数：**收尾**一些关闭的回调函数，如：socket.on('close', ...)。

这些阶段，维护了多个队列。通常情况下，node 停留在 轮询 IO 操作，监听并等待新的 IO 请求。这些队列都是宏任务队列。

![截屏2022-08-02 00.33.39](images/03%E9%A1%B5%E9%9D%A2%E5%BE%AA%E7%8E%AF%E7%B3%BB%E7%BB%9F.assets/%E6%88%AA%E5%B1%8F2022-08-02%2000.33.39.png)



#### 2. Node 的宏任务/微任务

一次事件循环的Tick，Node的事件循环更复杂，它也分为微任务和宏任务： 
...
和浏览器一样，在执行任何宏任务之前，要确保微任务队列已经全部执行完毕。
...
- other queue：Promise的 then 回调、queueMicrotask； 
...
❗️注意：在每一次事件循环的 tick 中，会按照如下顺序来执行代码：

- （微）next tick microtask queue； 
- （微）other microtask queue； 
- timer queue； 
- poll queue； 
- check queue； 
- close queue；
